...
oscill(o) = os.oscsin(frq*avo) :  hgroup("[02] OSC %avo",  *(vol)  <:  *(sqrt(1-pan)), *(sqrt(pan)) )
  with{ 
    avo = o+(001);
    ...
    vol = oscgroup(vslider("[02] VOL [style:knob]", 0,-70,0,0.01));
 };

stereo = hgroup("[127] STEREO OUT", *(vol), *(vol) : vmeter, vmeter)
  with{
    vol = vslider("[01] VOL ", 0.0,-70,+6,0.1) : ba.db2linear : si.smoo;
    };
